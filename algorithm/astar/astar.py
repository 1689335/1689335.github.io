...
class AStar:
    ...
    def show_img(self):
        #dim = (self.img.shape[1] * 8, self.img.shape[0] * 8)
        dim = (self.width * 8, self.height * 8)
        resize_img = cv2.resize(self.img, dim) #interpolation=cv2.INTER_CUBIC)
        cv2.imshow("path", resize_img)
        cv2.waitKey(100000)

    def astar_search(self, map, start, end):
        self.img[start[0]][start[1]] = (0, 0, 255)
        self.img[end[0]][end[1]] = (0, 0, 255)
        #self.show("path", self.img)
...
def main():
    ...
    astar = AStar(map, width, height)
    path = astar.astar_search(map, start, end)
    print()
    print(path)
    print()
